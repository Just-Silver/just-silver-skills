...
- 一键安装/卸载：Git Bash 命令窗粘贴 `curl -fsSL https://raw.githubusercontent.com/Just-Silver/just-silver-skills/main/scripts/install-skills.sh | bash` 安装、`.../uninstall-skills.sh | bash` 卸载（脚本在 `scripts/install-skills.sh` / `scripts/uninstall-skills.sh`）；远程拉 main.tar.gz → 原子替换全局 `skills/just-silver-skills/`，不动他人技能，幂等可重跑。两脚本都内置 MSYS 路径修正（PowerShell 里 `curl | bash` 时 bash 继承 Windows PATH，需前置 `/usr/bin` 并把 `$DEST` 经 cygpath 转 Unix 路径，否则 `find`/glob 会踩 Windows 工具）
